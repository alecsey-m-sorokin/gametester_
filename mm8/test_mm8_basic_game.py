...
from Locators import bets, APIdata_TwinWins
from TwinWins_Page import API_TwinWins, Logger, Reddy
...
FS_TWIN_collected_count = []
FS_BONUS_collected_count = []
FS_BONUS_collected = []
FS_TWIN_collected_winnings = []
FS_BONUS_collected_winnings = []

FS_TWIN_collected_count.clear()
FS_BONUS_collected_count.clear()
FS_TWIN_collected_winnings.clear()
...
while r < sessions:  # set the number of rounds (sessions)
    ...
    if resultId is not None:  # if ResumeGame
        ...
        if totalFreeSpinsCount:
            ...
            print2(f'\n----- Twin Wins {fsLabel} free spin # {str(totalFreeSpinsCount - remainingFreeSpinsCount)}')
            globalWinsFS.clear()
            ...
            FS_TWIN_collected_winnings.append(sum(globalWinsFS) * coin)  # тут сохраняем сколько игрок выиграл в CURRENCY за totalFreeSpinsCount фри спинов
        else:
            pass
    ...
    while i < rounds:  # set the number of spins
        ...
        if totalFreeSpinsCount:
            ...
            print2(str(getAsyncResponseFreeSpin))
            globalWinsFS.clear()
            FS_TWIN_collected_count.append(totalFreeSpinsCount)  # сюда помещаем значения totalFreeSpinsCount, которые получает Игрок
            globalWinsFS.append(getAsyncResponse["WinInfo"]["CurrentSpinWin"])  # тут добавляем выигрыш с основного раунда перед фри спинами
            globalWinsFS.append(getAsyncResponseFreeSpin["WinInfo"]["CurrentSpinWin"])
            print2('Current freeSpin win = ', getAsyncResponseFreeSpin["WinInfo"]["CurrentSpinWin"])
            print2('globalWinsFS = ', globalWinsFS)
    ...
    print2('\n')
    print2(f'finished "Twin Wins" session after {i} spins')
    ...
    text_bot_1 = f'finished "Twin Wins" session after {rounds} spins \n UserId = {A.userID} \n totalWins = {totalWins} \n sum totalWins = {sum(totalWins)} \n ' \
...
text_bot_2 = f'finished "Twin Wins" after {sessions} sessions with {rounds} rounds\n UserId = {A.userID} \n total bets = {sum(globalBets) * coin} \n globalWins = {globalWins} \n ' \
