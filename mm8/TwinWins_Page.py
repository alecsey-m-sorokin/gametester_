...
import pytest
import requests
from Locators import APIdata_TwinWins, ErrorCodes
    ...
    def GetAsyncResponse(RegToken, TimeOut, TokenAsync):
        ...
        while "Error" in response:
            ...
            if 'UserSavedState' not in response["SpinResult"]:
                print('no TwinWins Save State ...')
            else:
                print('-TwinWins Save State-----')
                bonusGameResult = findall(response, "BonusGame")
                print('BonusGame = ', bonusGameResult)
                if bonusGameResult is not None:
                    print(bonusGameResult)
